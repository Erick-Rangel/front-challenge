...
const App = () => {
  ...
  useEffect(()=>{
    ...
    .then((data)=>{
      const componentsData = data?.map((i)=>{
        return{
          id: i.id,
          title: i.title,
  ...
  const addMoveable = () => {
    ...
    const newMoveable = {
      ...
      width: 150,
      height: 150,
      backgroundImage: `url(${components[moveableComponents.length]?.imgUrl})`,
      };
    setMoveableComponents([...moveableComponents, newMoveable]);
  };
  console.log("moveableComponents", moveableComponents)
  ...
  const handleResizeStart = (index, e) => {
    ...
    if (handlePosX === -1) {
      ...
      if(handlePosY === -1){
        ...
        e.set([null,initialTop,null,initialHeight])
      }  
    };
    ...
    <>
      ...
      <Moveable
        ...
        padding={{ left: 0, top: 0, right: 0, bottom: 0 }}
                >
        {/* eliminar moveable */}
        <button onClick={() => deleteMoveable(id)}>X</button>
      </Moveable>
    </>
  );
};
